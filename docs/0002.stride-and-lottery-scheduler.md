...
void scheduler(void) {
    ...
    p = get_min_pass_proc_locked();              // 최소 pass value 프로세스 찾기
    if(p) {
        p->state = RUNNING;
...
#### Pass Value 시스템 (deprecated)

-   새로운 프로세스가 생성되었을 때, pass_value 를 어떻게 설정해야할까? 더 고민해봐야함.
-   현재는 로또 스케줄러만 구현하도록 다시 수정하였는데, 0 이 아니라 min_passvalue 으로 설정할 것 같다.
